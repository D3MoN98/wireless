...
class Plan extends CI_Model
{
    ...
    {
        return $this->db->select('service_types.name as service, plans.name as plan_name, service_providers.name as service_provider_name, plans.*, service_types.*, service_providers.*')
        ->from('plans')
        ->join('service_types', 'plans.service_type_id = service_types.service_type_id')
        ->join('service_providers', 'plans.user_id = service_providers.user_id')
        ->where("FIND_IN_SET('$state_id', `state_id`)")
    ...
    public function get_plans_by_state_id_api($zip,$state,$state_id,$tribal)
    {
        // return $this->db
        //     ->select('company,carrier_type')
        //     ->from('ServiceProviders_From_Api')
        //     ->join('service_types', 'plans.service_type_id = service_types.service_type_id')
        //     ->join('service_providers', 'plans.user_id = service_providers.user_id')
        //     ->join('payment_methods', 'payment_methods.user_id = plans.user_id')
        //     ->where("carrier_type!='Wireline'")
        //     ->where("FIND_IN_SET('$state_id', `state_id`)")
        //     ->where(['tribal_plan' => $tribal])
        //     ->where(['is_active' => 1])
        //     ->where(['payment_methods.is_validated' => 1])
        ...
        // // return $this->db->query("SELECT company,carrier_type FROM `ServiceProviders_From_Api` WHERE carrier_type!='Wireline' limit 3 " )->result();
        return $this->db->query("SELECT company,carrier_type FROM `ServiceProviders_From_Api` WHERE carrier_type!='Wireline' and city like '$state %' and  zip_code = '$zip'  " )->result();

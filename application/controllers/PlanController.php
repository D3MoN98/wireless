...
class PlanController extends CI_Controller
{
	...
	public function search_plans($state_id)
	{
		...
		if (isset($_SESSION['api_service'])) {
			unset($_SESSION['api_service']);
		}
		...
		//echo $zip;
		// $data['api_service'] = $this->plan->get_plans_by_state_id_api($zip,$state_name,$state_id,$tribal);
		$url = "https://opendata.usac.org/resource/kjtb-4uf7.json?carrier_type=Wireless&zip_code=$zip";
		// echo $url;

		// die;
		$array_simple = (json_decode(json_encode($data['plans']), true));

		foreach ($array_simple as $value) {
			$service_provider_arr[] = $value['service_provider_name'];
		};
		$curl = curl_init($url);
		...
		if (count($data_api) > 0) {
			foreach ($data_api as $value) {
				if (strpos($value->city, $state_name) !== false) { //PAY ATTENTION TO !==, not !=
					if (!in_array($value->company, $service_provider_arr)) {
						$api_service[] = ['carrier_type' => $value->carrier_type, 'company' => $value->company, 'website' => $value->website];
						// $data_2[]=$value->company;
						$count++;
					}
				}
			}
		...
		$_SESSION['api_service'] = $api_service;
	...
	public function sort_plan_consumer($offset = 0)
	{
		...
		$api_service = $_SESSION['api_service'];
		$data['api_service'] = $api_service;
		$data['plans'] = $sorted_plans;
	...
	public function view_carriers()
	{
		...
		if (isset($zip_code) && $zip_code != '') {
			...
			// For total data
			// $url = 'https://opendata.usac.org/resource/kjtb-4uf7.json?$where=carrier_type%20in(%22Wireless%22,%20%22ACP%20Mobile%20Internet%22)&$order=company&zip_code=' . $zip_code;

			// $data['total_data'] = json_decode($this->apiCall($url));

			$url = 'https://opendata.usac.org/resource/kjtb-4uf7.json?$where=carrier_type%20in(%22Wireless%22,%20%22ACP%20Mobile%20Internet%22)&$order=company&zip_code=' . $zip_code;

			$data['carriers'] = json_decode($this->apiCall($url));
		}


		$this->load->view('carriers', $data);
	}
